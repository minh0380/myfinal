...
public class Paging {
	...
	//입력받을 값 >> getter,setter 다 필요
	private String type; //페이징 처리할 항목
	private int currentPage; //현재 페이지
	private int total; //전체 게시물 수
	private int cntPerPage; //페이지당 게시물 수
	private int blockCnt; //한 줄에 표시될 블록개수
	
	//계산해야하는 값 >> setter는 필요 없음
	...
	//쿼리에서 between문에 지정할 rownum값 >> setter는 필요 없음
	...
	private void calBlockStartAndEnd() {
		...
		//현재 페이지와 blockCnt 값이 같은 경우를 위해 이렇게 계산함.
